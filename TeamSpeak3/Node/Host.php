...
class Host extends AbstractNode
{
    ...
    public function permissionTree()
    {
        ...
        foreach ($this->permissionCats() as $key => $val) {
            ...
            $permtree[$val]["permcatname"] = HelperString::factory(
    ...
    public function whoamiSet($ident, $value = null)
    {
        ...
        $this->whoami[$ident] = (is_numeric($value)) ? intval($value) : HelperString::factory($value);
    }
    ...
    protected function fetchPermissionCats()
    {
        ...
        foreach ($reflects->getConstants() as $key => $val) {
            if (!HelperString::factory($key)->startsWith("PERM_CAT") || $val == 0xFF) {
                continue;
            }
